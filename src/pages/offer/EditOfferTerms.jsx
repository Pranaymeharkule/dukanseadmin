...
const EditOfferTerms = () => {
  ...
  // When switching to edit mode, set rawContent from description
  useEffect(() => {
    if (isEditing && termsData?.description) {
      setEditableContent(termsData.description);
      setRawContent(termsData.description.replace(/<[^>]*>/g, "")); // strip HTML
    }
  }, [isEditing, termsData]);
  ...
  const handleCancel = () => {
    setIsEditing(false);
    setEditableContent(termsData?.description || "");
    setRawContent(termsData?.description.replace(/<[^>]*>/g, "") || "");
  };
